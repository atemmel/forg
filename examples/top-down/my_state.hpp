...
struct MyState : public State {
  ...
  auto update() -> bool override {
    ...
    if (!is_walking) {
      ...
      switch (link_dir) {
      case Up:
        link_frame.x = walking_up.x + (link_anim_step * offset_frame_x);
        link_frame.y = walking_up.y;
        break;
      case Down:
        link_frame.x = walking_down.x + (link_anim_step * offset_frame_x);
        link_frame.y = walking_down.y;
        break;
      case Left:
        link_frame.x = walking_left.x + (link_anim_step * offset_frame_x);
        link_frame.y = walking_left.y;
        break;
      case Right:
        link_frame.x = walking_right.x + (link_anim_step * offset_frame_x);
